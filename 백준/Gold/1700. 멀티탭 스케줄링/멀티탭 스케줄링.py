import sys

N, K = map(int, input().split())
products = list(map(int, input().split()))
muliti_tab = [0] * N
ans = 0
change = maximum = 0

while products:
    product = products[0]
    # 멀티 탬에 동일한 제품이 꽂혀있을 경우 -> pass
    if product in muliti_tab:
        products.remove(product)
        continue
    # 멀티탭에 빈자리가 있을 경우
    elif 0 in muliti_tab:
        muliti_tab[muliti_tab.index(0)] = product
        products.remove(product)

    # 멀티탭에 빈자리가 없을 경우
    else:
        for mt in muliti_tab:
            # 멀티탭에 꽂혀있는 제품 중 이후에 꽂는 제품이 없는 경우
            # 이후에 꽂는 제품이 없는 제품을 빼주고 탐색중인 제품을 꽂는다.
            if mt not in products:
                change = mt
                break

            # 멀티탭에 꽂혀있는 제품 중 가장 나중에 사용하는 제품을 고름
            # -> 가장 나중에 사용하는 제품을 뽑고 탐색하고 있는 제품을 꽂는다.
            elif products.index(mt) > maximum:
                maximum = products.index(mt)
                change = mt

        muliti_tab[muliti_tab.index(change)] = product
        products.remove(product)
        maximum = 0
        ans += 1

print(ans)
